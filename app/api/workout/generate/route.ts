...
export async function POST(req: Request) {
  try {
    ...
    // ✅ UPDATED: added bodyFocus
    const { workoutType, bodyFocus } = await req.json();
    ...
    const exerciseList = availableExercises
      .map((e) => `${e.id} (${e.name}, targets: ${e.muscleGroup})`)
      .join(", ");

    // Recent muscles logic
    ...
    const avoidInstruction =
      uniqueRecentGroups.length > 0
        ? `IMPORTANT: User worked these muscle groups recently — try to AVOID them today: ${uniqueRecentGroups.join(
            ", "
          )}. EXCEPTION: if goal is belly_fat or core, abs/core exercises can always be included.`
        : "No recent workout data — create a balanced full-body workout.";

    // ✅ NEW: Focus instruction
    const focusInstruction =
      bodyFocus && bodyFocus !== "All"
        ? `IMPORTANT: Focus exercises on ${bodyFocus}. Prioritize ${bodyFocus}-targeting exercises.`
