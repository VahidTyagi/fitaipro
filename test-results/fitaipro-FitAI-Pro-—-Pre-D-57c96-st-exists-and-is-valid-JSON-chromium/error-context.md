...
- Name: fitaipro.spec.ts >> FitAI Pro — Pre-Deploy Suite >> T10 — PWA manifest exists and is valid JSON
- Location: tests\e2e\fitaipro.spec.ts:157:7

# Error details

```
Error: manifest.json should return JSON not HTML

expect(received).not.toContain(expected) // indexOf

Expected substring: not "text/html"
...
> 164 |     expect(contentType, "manifest.json should return JSON not HTML").not.toContain("text/html");
  ...
  253 |     const body = await page.textContent("body");
  254 |     expect(body).toMatch(/Progress|Weight|Track|Log/i);
  255 |   });
  256 | 
  257 |   // ── T18 ──────────────────────────────────────────────────────────────────────
  258 |   test("T18 — Auth: Settings page loads", async ({ page }) => {
  259 |     await signInTestUser(page);
  260 |     await page.goto(`${BASE}/dashboard/settings`, { waitUntil: "domcontentloaded" });
  261 |     await page.waitForTimeout(2000);
  262 |     const body = await page.textContent("body");
  263 |     expect(body).toMatch(/Settings|Profile|subscription|Subscription/i);
  264 |   });
```